...
#exec watchmedo auto-restart -d /usr/lib/python3.6/site-packages/f8a_worker -p "*.py;*.sh" --recursive -- celery worker -P solo -A f8a_worker.start -Q "${WORKER_QUEUES}" --concurrency=1 --prefetch-multiplier=128 -Ofair --without-gossip --without-mingle --without-heartbeat
