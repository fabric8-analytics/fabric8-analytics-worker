...
class GraphPopulator(object):
    ...
    def construct_version_query(cls, input_json):
        ...
        str_version += "ver = g.V().has('pecosystem', ecosystem).has('pname', pkg_name)" \
                       ".has('version', version).tryNext().orElseGet{graph.addVertex('pecosystem', ecosystem," \
                       "'pname', pkg_name, 'version', version, " \
    ...
    def construct_package_query(cls, input_json):
        pkg_name_tokens = re.split('\W+', input_json.get('package'))
        prp_package = ""
    ...
    def create_query_string(cls, input_json):
        ...
        if input_json.get('version') is not None and input_json.get('version') != '':
            ...
            str_gremlin += "g.V().has('pecosystem', ecosystem).has('pname', pkg_name)." \
                           ...
                           ".tryNext().orElseGet{graph.addVertex('ecosystem', ecosystem, 'name', pkg_name, " \
                           "'vertex_label', 'Package')};g.V(pkg).next().addEdge('has_version',ver)};"
