...
can check the structure in `cucoslib.defaultconf`. Configuration is read in following order, where
latter overwrites former (see CucosConfiguration._default_backends):

 1. default configuration read from `cucoslib.defaultconf`
...
configuration = None
logger = logging.getLogger("cucoslib")
